...
受保护、可信服务器和平均速率字段只读。分步间隔默认 120 秒，最低可选 60 秒；Steam 未公开保证通过的最快间隔。停止只阻止后续轮次，不能撤销已发请求或 Steam 缓存改动。

运行日志自动保存到 `SteamStatsEditor.exe` 同级的 `logs/`，日志窗口显示完整文件路径。每次打开游戏编辑窗口会新建日志文件，文件不会自动删除或上传；无需保留时可在关闭程序后手动删除。程序目录不可写时会提示自动保存失败，可从日志窗口手动另存。旧版 `%LOCALAPPDATA%/SteamStatsEditor/logs/` 中的日志保持原样，不会自动迁移或清理。窗口显示最多约 20,000 条记录，其显示裁剪不影响磁盘日志。
